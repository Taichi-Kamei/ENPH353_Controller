        ...
        if len(contour_data) >= 2:
            ...
            if M_left["m00"] > 0 and M_right["m00"] > 0:
                ...
                if abs(cx_left - cx_right) <= 300:
                    ...
                    left_bottom_ok  = (yL + hL) >= frame_height - bottom_tol
                    right_bottom_ok = (yR + hR) >= frame_height - bottom_tol

                    left_in_range  = yL <= 0.5 * frame_height
                    right_in_range = yR <= 0.5 * frame_height

                    left_is_left   = xL <= 0.2 * frame_width
                    right_is_right = xR >= 0.85 * frame_width    
